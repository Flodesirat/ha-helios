...
# ---------------------------------------------------------------------------
# Old dispatch (kept for backward compatibility — used by tests and optimizer)
# ---------------------------------------------------------------------------
...
def _is_satisfied(dev: SimDevice, managed: _ManagedDevice | None, sim_now: _RealDatetime) -> bool:
...
def _must_run(dev: SimDevice, managed: _ManagedDevice | None, sim_now: _RealDatetime) -> bool:
...
def dispatch(
    ...
    sim_now: _RealDatetime | None = None,
) -> None:
    ...
    """
    _now = sim_now or _RealDatetime.now()
